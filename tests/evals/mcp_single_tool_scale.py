    ...
    def _inner():
        ...
        try:
            ...
            with csv_path.open("a", newline="") as f:
                ...
                if header_needed:
                    w.writerow(
                        ["worker", "label", "start_unix", "end_unix", "elapsed_s"]
                    )
                w.writerow(
                    [
                        worker_id,
...
def main():
    ...
    p.add_argument(
        "-n", "--calls", type=int, default=10, help="number of calls per worker"
    )
    args = p.parse_args()
    ...
    for wid in range(1, args.workers + 1):
        p = multiprocessing.Process(
            target=worker_main, args=(wid, args.url, args.calls, csv_path)
        )
        p.start()
        procs.append(p)
    for p in procs:
